...
const Section5 = ({ selectedBU, accessData, setAccessData, user }) => {
  ...
  useEffect(() => {
    if (user.permissions) {
      const temp = {};
      user.permissions.region_brand?.forEach((zone) => {
        zone.countries.forEach((country) => {
          temp[country.country_name] = country.brands.map(
      ...
      <Box bg="#f2f2f2" width="98%" margin="auto">
        <SimpleGrid columns={6} width="100%" pt="2rem" px="1rem" pb="0rem">
          {brands.map((item, i) => (
            <Flex
              bg="white"
              fontSize={item.brand?.length > 40 ? "0.7rem" : "0.875rem"}
              lineHeight="1.2"
              borderRadius="8px"
              userSelect="none"
              cursor="pointer"

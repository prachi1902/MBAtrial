...
const VehicleMonthlyChart = ({ boxProps, name, dependencies = [] }) => {
  ...
  useMemo(() => {
    if (vehicleValue?.value) {
      ...
      if (mediaPlanState.selectedPlan.type === "opt") {
        mediaPlanState.selectedPlan.spends.forEach((plan) => {
          if (plan.vehicle === vehicleValue.value) {
            if (spend[plan.month?.toUpperCase()]) {
              spend[plan.month?.toUpperCase()] += parseFloat(
            ...
            if (volume[plan.month?.toUpperCase()]) {
              volume[plan.month?.toUpperCase()] += parseFloat(
      ...
      tooltip.titleFormat(
        "<div style='margin-bottom: 0.5rem;'><p style='text-align: center;'>Month {%x}</p></div>"
      );

      tooltip.format(
        "<div class='flex-div' style='width: 85%; justify-content: flex-start; margin-bottom: 0.6rem;'><div style='display: flex; align-items: center; margin-right: 6.65rem;'><div class='green-circle'></div><p>{%seriesName}</p></div><p style='font-weight: 600;'> $ {%value}{groupsSeparator:\\,, decimalsCount:2}</p></div>"
      );

      var diffTooltip = series2.tooltip();
      diffTooltip.format(
        "<div class='flex-div' style='width: 85%; justify-content: flex-start;'><div style='display: flex; align-items: center; margin-right: 7.15rem;'><div class='color-box'></div><p>{%seriesName}</p></div><p style='font-weight: 600;'>{%value}{groupsSeparator:\\,, decimalsCount:2} HL</p></div></div>"
      );
...
export default VehicleMonthlyChart;

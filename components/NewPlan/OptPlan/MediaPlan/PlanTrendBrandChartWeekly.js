...
const StackedChart = ({ boxProps, name, dependencies = [] }) => {
  ...
  useMemo(() => {
    ...
    if (mediaPlanState.selectedPlan.type === "opt") {
      mediaPlanState.selectedPlan.spends?.forEach((plan) => {
        if (weeks[plan.week]) {
          ...
          spendsTotal[plan.week] += parseFloat(plan.opt_planned_spend);
          if (weeks[plan.week][plan.vehicle]) {
            weeks[plan.week][plan.vehicle] += parseFloat(
    ...
    chart.xScroller().thumbs().enabled(false);
    chart.xScroller().maxHeight(13);

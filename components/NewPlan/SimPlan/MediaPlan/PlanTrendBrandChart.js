...
const StackedChart = ({ boxProps, name, dependencies = [] }) => {
  ...
  useEffect(() => {
    ...
    mappingData.forEach((plan) => {
      if (months[plan.month]) {
        volumeData[plan.month] += parseFloat(plan.final_adjusted_volume);
        spendsTotal[plan.month] += parseFloat(plan.opt_planned_spend);
        if (months[plan.month][plan.vehicle]) {
          months[plan.month][plan.vehicle] += parseFloat(
            plan.opt_planned_spend
          );
        } else {
          months[plan.month][plan.vehicle] = parseFloat(plan.opt_planned_spend);
        }
      } else {
        volumeData[plan.month] = parseFloat(plan.final_adjusted_volume);
        spendsTotal[plan.month] = parseFloat(plan.opt_planned_spend);
    ...
    const formatted = Object.keys(volumeData).map((month) => {

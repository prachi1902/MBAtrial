...
const StackedChart = ({ boxProps, name, dependencies = [] }) => {
  ...
  useEffect(() => {
    ...
    var seriesData_12 = data.mapAs({ x: 0, value: 11 });
    ...
    // var yScale1 = anychart.scales.linear();
    // yScale1.minimum(0);
    // yScale1.maximum(10000);

    var yScale2 = anychart.scales.linear();
    yScale2.stackMode("value");
    yScale2.minimum(0);
    yScale2.maximum(320000);

    // var series12 = chart.area(seriesData_12).yScale(yScale1);
    ...
    while (chart.getSeriesAt(i + 1)) {
      var individualTooltip = chart.getSeriesAt(i + 1).tooltip();
      individualTooltip.format(
        "<div class='flex-div' style='width: 47%; margin: 0 auto; margin-bottom: 0.3rem;'><div style='display: flex; align-items: center;'><div class='green-circle' style='background-color:" +
          chart.getSeriesAt(i + 1).color() +
          ";'></div><p>{%seriesName}</p></div><p>{%value}{decimalsCount:2}%</p></div>"
      );
      i++;
    }
